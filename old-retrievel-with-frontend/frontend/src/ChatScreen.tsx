...
import type { UserRole, SourceChunk, ChatResponse } from './api';
...
function stageLabel(stage: string): { text: string; status: 'done' | 'blocked' | 'error' | 'info' } {
  if (stage.startsWith('safety_blocked')) return { text: '🚫 حجب الأمان', status: 'blocked' };
  if (stage === 'safety_passed') return { text: '🛡️ أمان', status: 'done' };
  if (stage === 'topic_blocked') return { text: '🚫 خارج النطاق', status: 'blocked' };
  if (stage === 'topic_passed') return { text: '✅ الموضوع', status: 'done' };
  if (stage === 'embedded') return { text: '🔢 التضمين', status: 'done' };
  if (stage.startsWith('retrieved_')) return { text: `🔍 ${stage.split('_')[1]} نتيجة`, status: 'done' };
  if (stage.startsWith('reranked_')) return { text: `🎯 أفضل ${stage.split('_')[1]}`, status: 'done' };
  if (stage === 'rerank_fallback') return { text: '⚠️ ترتيب احتياطي', status: 'info' };
  if (stage === 'generated') return { text: '🤖 الإجابة', status: 'done' };
  if (stage === 'response_safe') return { text: '🛡️ آمن', status: 'done' };
  if (stage.startsWith('response_blocked')) return { text: '🚫 حُجبت الإجابة', status: 'blocked' };
  return { text: stage, status: 'info' };
}
